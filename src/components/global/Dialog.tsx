...
export const Dialog = (props: Props) => {
    const { children, className, open, onClose = () => { }, mobileFull = false, tabletFull = false, ...others } = props;
    ...
    return (
        <div className={open ? 'av-dialog-wrapper open' : 'av-dialog-wrapper closed'}>
            ...
            <div className='av-dialog-container'>
                <div className={className ? `${dialogClass} ${className}` : dialogClass} ref={wrapper} {...others}>
                    {children}
                    <div className='av-dialog-close'>
                        <IconButton className='v-text' onClick={() => onClose()}>
                            <Icon name="Cross" />
                        </IconButton>
                    </div>
                </div>
