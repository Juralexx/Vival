...
export const paginate = (array, size) => {
    const arr = array.sort((a, b) => { return new Date(b[field]) - new Date(a[field]) })
    return divideArrayIntoSizedParts(arr, size)
}
...
/**
 * Sort by chronological dates
 * @param {*} array Array to sort
 * @param {*} field Fields used to sort
 */

export const sortByRecent = (array, field) => {
    const arr = array.sort((a, b) => { return new Date(b[field]) - new Date(a[field]) })
    return arr
}

/**
 * Sort by unchronological dates
 * @param {*} array Array to sort
 * @param {*} field Fields used to sort
 */

export const sortByOld = (array, field) => {
    const arr = array.sort((a, b) => { return new Date(a[field]) - new Date(b[field]) })
    return arr
}
...
/**
 * Replace all possibly accentuated characters, white spaces and hyphens to match partial search (useful for MongoDB partial search).
...
export const highlightSearchResults = (query, classname) => {
    ...
    let regex = new RegExp(convertStringToRegexp(escapeRegexp(query)), 'i');
